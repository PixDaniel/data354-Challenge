...
class CorrectiveRAG:
    ...
    def __init__(self, vector_store_path="src/data/faiss_index"):
        ...
        # Modèles
        self.llm = GoogleGenerativeAI(model="gemini-1.5-pro-latest", google_api_key=self.api_key, temperature=0)
    ...
    def ask_for_eval(self, question: str) -> dict:
        """
        Pose une question et retourne la réponse ainsi que le contexte,
        pour l'évaluation.
        """
        inputs = {"question": question}
        result = self.graph.invoke(inputs, {"recursion_limit": 5})
