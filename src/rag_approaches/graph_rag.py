...
class GraphRAG:
    ...
    def __init__(self):
        ...
        self.llm = GoogleGenerativeAI(model="gemini-1.5-pro-latest", google_api_key=GEMINI_API_KEY, temperature=0)
    ...
    def ask_for_eval(self, question: str) -> dict:
        """
        Pose une question et retourne la réponse ainsi que le contexte du graphe,
        pour l'évaluation.
        """
        result = self.chain.invoke({"query": question})
        answer = result.get("result", "")
        intermediate_steps = result.get("intermediate_steps", [{}])
        # Le contexte est le résultat brut de la requête cypher
        raw_context = intermediate_steps[0].get("context", []) if intermediate_steps else []
        contexts = [str(item) for item in raw_context] if raw_context else []
